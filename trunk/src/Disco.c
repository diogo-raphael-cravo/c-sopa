...
* @param int				bloco_param		Para uma carga, o bloco que será carregado.
*/
void privada_executar(DISCO *disco_param, OPERACAO_DISCO operacao_param, int endereco_param, PALAVRA dados_param, int bloco_param){
	if(	((operacao_param == OPERACAO_LEITURA_DISCO || operacao_param == OPERACAO_ESCRITA_DISCO)
			&& (endereco_param < 0 || TAMANHO_DISCO_PALAVRAS <= endereco_param))
		|| (operacao_param == OPERACAO_CARGA_DISCO && (bloco_param < 0 || QUANTIDADE_BLOCOS_DISCO <= bloco_param)) ){
...
void disco_rodar(DISCO *disco_param){
	...
	while(1){
		...
		privada_executar(disco_param, disco_param->proximaOperacao, 
			disco_param->enderecoProximaOperacao, disco_param->dadosProximaEscrita, disco_param->enderecoProximaOperacao);
...
*/
void disco_executarOperacao(DISCO *disco_param, OPERACAO_DISCO operacao_param, int enderecoOuBloco_param, PALAVRA dados_param){
	disco_param->tamanhoUltimaLeitura = 0;
	disco_param->erroUltimaOperacao = SEM_ERRO_DISCO;
